...
public class PlanEjecucionReporteAction extends VgcReporteBasicoAction {
	...
	protected void construirReporte(ActionForm form, HttpServletRequest request, HttpServletResponse response,
		...
		if (source.equals("Plan"))
			...
			EjecucionIniciativa(reporte, documento, request, mensajes, source, todos);
	}
	...
	private void EjecucionIniciativa(ReporteForm reporte, Document documento, HttpServletRequest request,
			MessageResources mensajes, String source, String todos) throws Exception {
		// Raiz del plan
		lineas = 2;
		...
		/*List<TipoProyecto> tipo = new ArrayList<TipoProyecto>();
		tipo = reporte.getTipos();
		for(int i=0; i<tipo.size(); i++) {
			TipoProyecto tipo2 = new TipoProyecto();
			tipo2 = reporte.getTipos().get(i);
			if(tipo2.getTipoProyectoId() == reporte.getTipo()) {				
				texto = new Paragraph("tipo de proyecto: " + tipo2.getNombre(), font);
				// texto = new Paragraph("tipo de proyecto: " + reporte.getTipo(), font);
				texto.setAlignment(Element.ALIGN_CENTER);
				documento.add(texto);
				lineas = getNumeroLinea((lineas + 5), inicioLineas);
			}
		...
		texto = new Paragraph("Estatus: " + reporte.getEstatus(), font);
		texto.setAlignment(Element.ALIGN_CENTER);
		documento.add(texto);
		lineas = getNumeroLinea((lineas + 5), inicioLineas);

		if (todos.equals("false")) {
			texto = new Paragraph("Año formulacion: " + reporte.getAno(), font);
			texto.setAlignment(Element.ALIGN_CENTER);
			documento.add(texto);
			lineas = getNumeroLinea((lineas + 5), inicioLineas);
		}else {
			texto = new Paragraph("Año formulacion: " + "Todos", font);
			texto.setAlignment(Element.ALIGN_CENTER);
			documento.add(texto);
			lineas = getNumeroLinea((lineas + 5), inicioLineas);
		}
		...
		if (reporte.getVisualizarIniciativas()) {
			...
			dibujarInformacionIniciativa(nivel, reporte, font, source, null, documento, strategosMedicionesService,
	...
	private void dibujarInformacionPerspectiva(int nivel, ReporteForm reporte, Font font, String source,
		...
		if (reporte.getVisualizarIniciativas())
			dibujarInformacionIniciativa(nivel, reporte, font, source, perspectiva, documento,
	...
	private void dibujarInformacionIndicador(int nivel, ReporteForm reporte, Font font, Perspectiva perspectiva,
		...
		Font fontT = new Font(getConfiguracionPagina(request).getCodigoFuente());
		fontT.setSize(12);
		...
		if (indicadores.size() > 0) {
			for (Iterator<Indicador> iter = indicadores.iterator(); iter.hasNext();) {
				...
				if (reporte.getVisualizarIndicadoresAlerta()) {
					...
					// Nombre
					tab.agregarCelda(
							reporte.getPlantillaPlanes().getNombreIndicadorSingular() + " : " + indicador.getNombre());
					lineas = getNumeroLinea(lineas, inicioLineas);
					documento.add(tab.getTabla());
					if (lineas >= tamanoPagina) {
						lineas = inicioLineas;
				...
				if (reporte.getVisualizarIndicadoresEstadoParcial())
					estadosParciales = strategosPlanesService.getIndicadorEstados(indicador.getIndicadorId(),
							reporte.getPlanId(), indicador.getFrecuencia(),
							TipoIndicadorEstado.getTipoIndicadorEstadoParcial(), new Integer(reporte.getAnoInicial()),
							new Integer(reporte.getAnoFinal()), periodoInicio, periodoFin);
				if (reporte.getVisualizarIndicadoresEstadoAnual())
					estadosAnuales = strategosPlanesService.getIndicadorEstados(indicador.getIndicadorId(),
							reporte.getPlanId(), indicador.getFrecuencia(),
							TipoIndicadorEstado.getTipoIndicadorEstadoAnual(), new Integer(reporte.getAnoInicial()),
							new Integer(reporte.getAnoFinal()), periodoInicio, periodoFin);
				...
				if (mediciones.size() != 0) {
					...
					crearTablaIndicador(indicador, reporte, mediciones, medicionesMetas, estadosParciales,
							estadosAnuales, reporte.getVisualizarIndicadoresEjecutado(),
							reporte.getVisualizarIndicadoresMeta(), reporte.getVisualizarIndicadoresEstadoParcial(),
							...
							reporte.getVisualizarIndicadoresAlerta(), font, mensajes, strategosMedicionesService,
							strategosIndicadoresService, documento, request);
					if (lineas >= tamanoPagina) {
						lineas = inicioLineas;
						tamanoPagina = inicioTamanoPagina;
						saltarPagina(documento, false, font, null, null, request);
					...
					font.setColor(0, 0, 255);
					texto = new Paragraph(
							mensajes.getMessage("jsp.reportes.plan.ejecucion.reporte.indicadores.nomediciones",
									reporte.getPlantillaPlanes().getNombreIndicadorSingular().toLowerCase()),
							font);
					texto.setIndentationLeft(50);
					documento.add(texto);
					font.setColor(0, 0, 0);
			...
			texto = new Paragraph(mensajes.getMessage("jsp.reportes.plan.ejecucion.reporte.noindicadores",
					...
					getNombrePerspectiva(reporte, (nivel)).toLowerCase()), font);
			texto.setIndentationLeft(50);
			documento.add(texto);
			font.setColor(0, 0, 0);
			lineas = getNumeroLinea(lineas, inicioLineas);
	...
	private void dibujarInformacionIniciativa(int nivel, ReporteForm reporte, Font font, String source,
			Perspectiva perspectiva, Document documento, StrategosMedicionesService strategosMedicionesService,
		...
		if (source.equals("Plan")) {
		...
		} else if (source.equals("Iniciativa") || source.equals("IniciativaGeneral")
				|| source.equals("IniciativaPlan")) {
			...
			if (reporte.getPlanId() != null) {
				...
				reporte.setPlantillaPlanes((PlantillaPlanes) strategosIniciativasService.load(PlantillaPlanes.class,
						new Long(plan.getMetodologiaId())));
			} else {
		...
		if (iniciativas.size() > 0) {
			for (Iterator<Iniciativa> iter = iniciativas.iterator(); iter.hasNext();) {
				...
				crearTablaIniciativa(reporte, iniciativa, indicador, medicionesProgramado, medicionesEjecutado, font,
						mensajes, documento, request);
				...
				if (medicionesEjecutado.size() != 0)
					crearTablaIndicador(indicador, reporte, medicionesEjecutado, medicionesProgramado,
							...
							reporte.getVisualizarIniciativasEjecutado(), reporte.getVisualizarIniciativasMeta(), false,
							false, false, false, reporte.getVisualizarIniciativasAlerta(), font, mensajes,
							strategosMedicionesService, strategosIndicadoresService, documento, request);
				else {
					documento.add(lineaEnBlanco(font));
					lineas = getNumeroLinea(lineas, inicioLineas);
	...
	private void crearTablaIniciativa(ReporteForm reporte, Iniciativa iniciativa, Indicador indicador,
			...
			MessageResources mensajes, Document documento, HttpServletRequest request) throws Exception {
		if (lineas >= tamanoPagina) {
			lineas = inicioLineas;
			tamanoPagina = inicioTamanoPagina;
			saltarPagina(documento, false, font, null, null, request);
	...
	private void crearTablaActividad(ReporteForm reporte, PryActividad actividad, Indicador indicador, Font font,
		...
		encabezado = encabezado
				+ mensajes.getMessage("jsp.reportes.plan.ejecucion.reporte.titulo.porcentaje.programado") + ",";
		encabezado = encabezado + mensajes.getMessage("jsp.reportes.plan.ejecucion.reporte.titulo.responsable");
