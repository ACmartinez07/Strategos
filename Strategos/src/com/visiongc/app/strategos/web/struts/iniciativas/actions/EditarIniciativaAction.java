	...
	public String getData(EditarIniciativaForm editarIniciativaForm, String forward, HttpServletRequest request)
	{
		...
		if ((iniciativaId != null) && (!iniciativaId.equals("")) && (!iniciativaId.equals("0")))
		{
			...
			if (iniciativa != null)
			{
				...
				for (Iterator<?> i = iniciativa.getResultadosEspecificosIniciativa().iterator(); i.hasNext(); ) 
				{
					...
					resultadosEspecificos = resultadosEspecificos + resultadoEspecificoIniciativa.getResultado() + editarIniciativaForm.getSEPARADOR();
					anoTemp++;
				}
				...
				estatusId = new com.visiongc.app.strategos.web.struts.planificacionseguimiento.actions.CalcularActividadesAction().CalcularEstatus(iniciativa.getPorcentajeCompletado());
			}
			else 
			{
		...
		{			
			...
			if(desdeInstrumento != null ) {
				if(!desdeInstrumento) {
					OrganizacionStrategos organizacion = (OrganizacionStrategos)strategosIniciativasService.load(OrganizacionStrategos.class, new Long((String)request.getSession().getAttribute("organizacionId")).longValue());
					editarIniciativaForm.setOrganizacionId(organizacion.getOrganizacionId());
					editarIniciativaForm.setOrganizacionNombre(organizacion.getNombre());			
				}
			}
		...
		if (editarIniciativaForm.getEstatusId() != null && editarIniciativaForm.getEstatusId().longValue() != EstatusType.getEstatusCencelado().longValue() && editarIniciativaForm.getEstatusId().longValue() != EstatusType.getEstatusSuspendido().longValue())
		...
		if (estatusProbable != null && editarIniciativaForm.getEstatusId() != null && (editarIniciativaForm.getEstatusId().longValue() == EstatusType.getEstatusCencelado().longValue() || editarIniciativaForm.getEstatusId().longValue() == EstatusType.getEstatusSuspendido().longValue()))
