...
public class StrategosIniciativasServiceImpl extends StrategosServiceImpl implements StrategosIniciativasService {
	private StrategosIniciativasPersistenceSession persistenceSession = null;

	public StrategosIniciativasServiceImpl(StrategosIniciativasPersistenceSession persistenceSession,
	...
	public PaginaLista getIniciativas(int pagina, int tamanoPagina, String orden, String tipoOrden, boolean getTotal,
			Map filtros) {
		return persistenceSession.getIniciativas(pagina, tamanoPagina, orden, tipoOrden, getTotal, filtros);
	}

	public PaginaLista getIniciativasResponsable(int pagina, int tamanoPagina, String orden, String tipoOrden,
			boolean getTotal, Map filtros) {
			...
			if ((iniciativa.getIniciativaId() == null) || (iniciativa.getIniciativaId().longValue() == 0L)) {
				if (persistenceSession.existsObject(iniciativa, fieldNames, fieldValues)) {
					...
					iniciativa.setEstatusId(
							com.visiongc.app.strategos.iniciativas.model.util.IniciativaEstatus.EstatusType
									.getEstatusSinIniciar());

					if (iniciativa.getResultadosEspecificosIniciativa() != null) {
						for (Iterator<ResultadoEspecificoIniciativa> i = iniciativa.getResultadosEspecificosIniciativa()
								.iterator(); i.hasNext();) {
					...
					if (iniciativa.getIniciativaPlanes() != null) {
						for (Iterator<IniciativaPlan> iter = iniciativa.getIniciativaPlanes().iterator(); iter
								.hasNext();) {
					...
					if (iniciativa.getIniciativaPerspectivas() != null) {
						for (Iterator<IniciativaPerspectiva> iter = iniciativa.getIniciativaPerspectivas()
					...
					if ((resultado == 10000) && (iniciativa.getIniciativaPerspectivas() != null)
							&& (iniciativa.getIniciativaPerspectivas().size() > 0)) {
						IniciativaPerspectiva iniciativaPerspectiva = (IniciativaPerspectiva) iniciativa
								.getIniciativaPerspectivas().toArray()[0];
						StrategosPerspectivasService strategosPerspectivasService = StrategosServiceFactory
								.getInstance().openStrategosPerspectivasService(this);
						...
						if (!strategosPerspectivasService.asociarIniciativa(perspectiva, iniciativa.getIniciativaId(),
								perspectiva.getPlanId(), iniciativa.getOrganizacionId(), usuario)) {
						...
						strategosPerspectivasService.close();
					} else if ((resultado == 10000) && (iniciativa.getIniciativaPlanes() != null)
							&& (iniciativa.getIniciativaPlanes().size() > 0)) {
						StrategosPlanesService strategosPlanesService = StrategosServiceFactory.getInstance()
								.openStrategosPlanesService(this);
						IniciativaPlan iniciativaPlan = (IniciativaPlan) iniciativa.getIniciativaPlanes().toArray()[0];
						resultado = strategosPlanesService.asociarIniciativa(iniciativaPlan.getPk().getPlanId(),
								iniciativa.getIniciativaId(), iniciativa.getOrganizacionId(), usuario);
				...
				if (persistenceSession.existsObject(iniciativa, fieldNames, fieldValues, idFieldNames, idFieldValues)) {
					...
					if (actualizarIndicador.booleanValue()) {
						if (iniciativa.getIndicadorId(TipoFuncionIndicador.getTipoFuncionSeguimiento()) != null) {
							...
							if (iniciativaOriginal.getFrecuencia().byteValue() != iniciativa.getFrecuencia()
									.byteValue()) {
								ConfiguracionIniciativa configuracionIniciativa = getConfiguracionIniciativa();
								resultado = updateIndicadorAutomatico(iniciativa,
								...
								if (resultado == 10000) {
									...
									if (indicadorId != null) {
										...
										if (iniciativa.getAlertaZonaVerde() != null)
											indicador.setAlertaMetaZonaVerde(
													new Double(iniciativa.getAlertaZonaVerde().doubleValue()));
										if (iniciativa.getAlertaZonaAmarilla() != null) {
											indicador.setAlertaMetaZonaAmarilla(
													new Double(iniciativa.getAlertaZonaAmarilla().doubleValue()));
										}
										configuracionIniciativa = getConfiguracionIniciativa();
										nombre = "";
										if (configuracionIniciativa.getIniciativaIndicadorAvanceAnteponer()
												.booleanValue())
	...
	public ConfiguracionIniciativa getConfiguracionIniciativa() {
		...
		try {
			...
			if (configuracion != null) {
				...
				if (nList.getLength() > 0) {
					for (int i = 0; i < nList.getLength(); i++) {
						...
						if (tipo.byteValue() == TipoFuncionIndicador.getTipoFuncionSeguimiento().byteValue()) {
							configuracionIniciativa.setIniciativaIndicadorAvanceNombre(
									VgcAbstractService.getTagValue("nombre", elemento));
							configuracionIniciativa.setIniciativaIndicadorAvanceMostrar(
									Boolean.valueOf(VgcAbstractService.getTagValue("crear", elemento).equals("1")));
							configuracionIniciativa.setIniciativaIndicadorAvanceAnteponer(
							...
							configuracionIniciativa.setIniciativaIndicadorPresupuestoNombre(
									VgcAbstractService.getTagValue("nombre", elemento));
							configuracionIniciativa.setIniciativaIndicadorPresupuestoMostrar(
									Boolean.valueOf(VgcAbstractService.getTagValue("crear", elemento).equals("1")));
						} else if (tipo.byteValue() == TipoFuncionIndicador.getTipoFuncionEficacia().byteValue()) {
							configuracionIniciativa.setIniciativaIndicadorEficaciaNombre(
									VgcAbstractService.getTagValue("nombre", elemento));
							configuracionIniciativa.setIniciativaIndicadorEficaciaMostrar(
									Boolean.valueOf(VgcAbstractService.getTagValue("crear", elemento).equals("1")));
						} else if (tipo.byteValue() == TipoFuncionIndicador.getTipoFuncionEficiencia().byteValue()) {
							configuracionIniciativa.setIniciativaIndicadorEficienciaNombre(
									VgcAbstractService.getTagValue("nombre", elemento));
							configuracionIniciativa.setIniciativaIndicadorEficienciaMostrar(
									Boolean.valueOf(VgcAbstractService.getTagValue("crear", elemento).equals("1")));
						}
					}
				}
				...
				configuracionIniciativa
						.setIniciativaNombre(messageResources.getResource("jsp.modulo.iniciativa.titulo.singular"));
				configuracionIniciativa.setIniciativaIndicadorAvanceNombre(
				...
				configuracionIniciativa.setIniciativaIndicadorPresupuestoNombre(messageResources
				...
				configuracionIniciativa.setIniciativaIndicadorEficaciaNombre(messageResources
				...
				configuracionIniciativa.setIniciativaIndicadorEficienciaNombre(messageResources
	...
	private int saveClaseIndicadores(Iniciativa iniciativa, Usuario usuario) {
		...
		ClaseIndicadores claseRoot = strategosClasesIndicadoresService.getClaseRaizIniciativa(
				iniciativa.getOrganizacionId(), TipoClaseIndicadores.getTipoClasePlanificacionSeguimiento(),
	...
	private int saveIndicadorAutomatico(Iniciativa iniciativa, Byte tipo,
	...
	private int crearIndicadorFormulaEficacia(Iniciativa iniciativa, Indicador indicador) {
		...
		if (iniciativa.getTipoMedicion().byteValue() == TipoMedicion.getTipoMedicionAlPeriodo().byteValue()) {
			formula = "([" + iniciativa.getIndicadorId(TipoFuncionIndicador.getTipoFuncionSeguimiento()).toString()
					+ ".0]" + "/" + "["
	...
	private int crearIndicadorFormulaEficiencia(Iniciativa iniciativa, Indicador indicador) {
		...
		if (iniciativa.getTipoMedicion().byteValue() == TipoMedicion.getTipoMedicionAlPeriodo().byteValue()) {
			formula =
					...
					"([" + iniciativa.getIndicadorId(TipoFuncionIndicador.getTipoFuncionSeguimiento()).toString() + "."
							...
							+ iniciativa.getIndicadorId(TipoFuncionIndicador.getTipoFuncionPresupuesto()).toString()
		...
		insumoFormula.getPk().setSerieId(SerieTiempo.getSerieReal().getSerieId());
		insumoFormula.getPk()
				.setIndicadorId(iniciativa.getIndicadorId(TipoFuncionIndicador.getTipoFuncionPresupuesto()));
	...
	public int updateIndicadorAutomatico(Iniciativa iniciativa, Byte tipo,
			ConfiguracionIniciativa configuracionIniciativa, Usuario usuario) {
		int resultado = 10000;
				...
				for (Iterator<?> i = dependencias.iterator(); i.hasNext();) {
					...
					if ((listaObjetosRelacionados.size() > 0)
						...
						strategosPryActividadesService.close();
							...
							&& ((listaObjetosRelacionados.get(0) instanceof PryProyecto))) {
						StrategosPryProyectosService strategosPryProyectosService = StrategosServiceFactory
								.getInstance().openStrategosProyectosService(this);
						...
						strategosPryProyectosService.close();
							...
							&& ((listaObjetosRelacionados.get(0) instanceof PrdProducto))) {
						StrategosPrdProductosService strategosPrdProductosService = StrategosServiceFactory
								.getInstance().openStrategosPrdProductosService(this);
	...
	public String getRutaCompletaIniciativa(Iniciativa iniciativa, String separador) {
		StrategosOrganizacionesService strategosOrganizacionesService = StrategosServiceFactory.getInstance()
				.openStrategosOrganizacionesService(this);
		String resultado = strategosOrganizacionesService
				.getRutaCompletaNombresOrganizacion(iniciativa.getOrganizacionId(), separador);
	...
	public List<Iniciativa> getIniciativasParaEjecutar(Long iniciativaId, Long organizacionId, Long planId,
			Integer ano) {
	...
	public CorreoIniciativa getCorreoIniciativa() {
		...
		try {
			FrameworkService frameworkService = FrameworkServiceFactory.getInstance().openFrameworkService();
			Configuracion configuracion = frameworkService
					.getConfiguracion("Strategos.Configuracion.Correo.Iniciativa");
			frameworkService.close();
			...
			if (configuracion != null) {
				...
				correoIniciativa.setEnviarResponsableCargarEjecutado(
						Boolean.valueOf(VgcAbstractService.getTagValue("cargarEjecutado", eElement).equals("1")));
				correoIniciativa.setEnviarResponsableCargarMeta(
						Boolean.valueOf(VgcAbstractService.getTagValue("cargarMeta", eElement).equals("1")));
				correoIniciativa.setEnviarResponsableFijarMeta(
						Boolean.valueOf(VgcAbstractService.getTagValue("fijarMeta", eElement).equals("1")));
				correoIniciativa.setEnviarResponsableLograrMeta(
						Boolean.valueOf(VgcAbstractService.getTagValue("lograrMeta", eElement).equals("1")));
				correoIniciativa.setEnviarResponsableSeguimiento(
						Boolean.valueOf(VgcAbstractService.getTagValue("seguimiento", eElement).equals("1")));
